...
WORKDIR /app
COPY ./ ./
WORKDIR /app/Vidload.Worker.DownloadService/
...
RUN apt-get install python3 python3-pip ffmpeg lame -y
RUN apt-get autoremove
RUN pip3 install youtube-dl

RUN mkdir /app
RUN mkdir -p /opt/mediastorage

WORKDIR /app
COPY --from=build_stage /app/publish .
CMD ["./Vidload.Worker.DownloadService"]
